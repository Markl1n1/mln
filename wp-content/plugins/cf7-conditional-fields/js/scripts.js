...
const Wpcf7cfForm = function ($form) {
  ...
  form.updateSimpleDom = function () {
    ...
    const inputs = Object.values(form.simpleDom).filter(
      (item) => item.type === "input",
    );
    const formdata = new FormData(form.$form[0]);

    let formdataEntries = [...formdata.entries()].map((entry) => [
      entry[0],
      entry[1].name ?? entry[1],
    ]);
    const buttonEntries = [...jQuery("button", form.$form)].map((entry) => [
      entry.name,
      entry.value,
    ]);
    ...
    inputs.forEach((simpleDomItem) => {
      const newValue = form.getNewDomValueIfChanged(
        simpleDomItem,
        formdataEntries,
      );
      if (newValue !== null) {
  ...
  form.isDomMatch = function (simpleDomItem, formDataEntries) {
    ...
    const currentValues = formDataEntries
      .filter((entry) => entry[0] === simpleDomItemName)
      .map((entry) => entry[1]);
    ...
    if (
      ...
      if ($group.prop("tagName") === "SPAN") {
        ...
        $group
          ...
          .trigger("wpcf7cf_show_group"); // show with animation
      }
      ...
      if ($group.attr("data-clear_on_hide") !== undefined) {
        const $inputs = jQuery(":input", $group).not(
      ...
      if ($group.prop("tagName") === "SPAN") {
        ...
        $group
          ...
          .trigger("wpcf7cf_hide_group"); // hide
      }
    }
  });
    ...
    if ($group.hasClass("wpcf7cf-hidden")) {
      ...
      if ($group.attr("data-disable_on_hide") !== undefined) {
        ...
        $group.find("input,select,textarea").each(function () {
          ...
          if (
            form.$form.find(
              `[data-class="wpcf7cf_group"]:not(.wpcf7cf-hidden) [name='${$this.attr("name")}']`,
            ).length === 0
          ) {
...
window.wpcf7cf = {
  ...
  updateRepeaterSubHTML: function (
    html,
    oldSuffix,
    newSuffix,
    parentRepeaters,
  ...
  get_simplified_dom_model: function (
    currentNode,
    simplified_dom = {},
    parentGroups = [],
    parentRepeaters = [],
    ...
    if (type) {
      ...
      const suffix = nameWithoutBrackets.replace(
      ...
      if (type === "input") {
        ...
        if (
          (currentNode.type === "checkbox" || currentNode.type === "radio") &&
          !currentNode.checked
        )
          return {};
    ...
    Array.from(children).forEach((childNode) => {
      const dom = wpcf7cf.get_simplified_dom_model(
        childNode,
        simplified_dom,
        newParentGroups,
        newParentRepeaters,
  ...
  updateMultistepState: function (multistep) {
    ...
    const $submit_button = multistep.form.$form
  ...
  should_group_be_shown: function (condition, form) {
    ...
    for (
      ...
      const $field =
        operator === "function" && jQuery(`[name="${inputField.name}"]`).eq(0);
  ...
  isConditionTrue(values, operator, testValue = "", $field = jQuery()) {
    ...
    const valuesAreEmpty =
      values.length === 0 || values.every((v) => !v && v !== 0); // 0 is not considered empty
    ...
    const testValueNumber = isFinite(parseFloat(testValue))
  ...
  getRepeaterObj($form, repeaterDataId) {
    ...
    const repeater = form.repeaters.find(
      (repeater) =>
        repeater.params.$repeater.attr("data-id") === repeaterDataId,
    );
